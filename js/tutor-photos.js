...
document.addEventListener("DOMContentLoaded", () => {
  ...
  //
  // Его рисует только renderPhotos, а тот вызывался ровно дважды: при
  // загрузке панели и по клику на вкладку. Репетитор держит панель
  // открытой весь урок — и всё это время видит цифру, снятую при входе,
  // обычно ноль. Ученик присылает фото тетради, а Ирина об этом не
  // узнаёт, пока не ткнёт во вкладку наугад.
  //
  // Тот же интервал, что у списка учеников (js/tutor.js, openPanel).
  // Не дёргаем сервер, когда вкладка браузера свёрнута: смотреть на
  // счётчик всё равно некому.
  setInterval(() => {
    if (document.hidden) return;
    if (!document.getElementById("photo-count")) return;
    loadPhotos();
  }, 60000);
});
