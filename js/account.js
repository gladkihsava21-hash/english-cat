...
async function loadAccount() {
  ...
  try {
    ...
    // Сервер — источник правды и по репетитору тоже: сохраняем имя,
    // чтобы следующая отрисовка (до ответа сервера) была верной сразу.
    if (typeof res.profile.tutorName === "string") {
      localStorage.setItem("savelyTutorName", res.profile.tutorName);
    }
...
function acTutorCard(p, online) {
   ...
   *
   * Пока ответ сервера не пришёл, карточка рисуется по локальным данным —
   * и раньше в этом случае она УТВЕРЖДАЛА «репетитора нет». Утверждение
   * было ложным: у axmat репетитор есть, просто метка на устройстве
   * пустая. Чужое имя в своём профиле увидеть неоткуда, а вот «я один»
   * вместо живого репетитора — обидная неправда, из-за которой человек
   * идёт спрашивать, куда делся его учитель.
   *
   * Поэтому: имя знаем — показываем; точно знаем, что репетитора нет
   * (ответ сервера пришёл) — говорим об этом; не знаем — так и пишем. */
  const answered = accountData !== null && accountData !== undefined;
  const state = p.tutorName ? "yes" : (online && !answered ? "unknown" : "no");
  return `
    <section class="card ac-card">
      ...
      ${state === "yes"
        ...
        : state === "unknown"
        ? `<p class="ac-note">Смотрю, кто твой репетитор…</p>`
...
function wireAccount(online) {
  ...
  if (claim) {
    claim.addEventListener("click", async () => {
      ...
      // Сеть отвалилась — говорим об этом ПРЯМО, а не «сохранено».
      //
      // registerStandalone при сетевом сбое возвращает {ok:true,
      // offline:true}, и проверка выше пропускала это как успех: ученик
      // читал «прогресс сохранён на сервере», уходил спокойным, а
      // аккаунта не было. Ни токена, ни личного кода, ни повторной
      // попытки — введённые почта и пароль просто пропадали. Дальше он
      // чистил историю или садился за другое устройство и терял всё.
      if (res.offline) {
        msg.textContent = "Сеть не отвечает — аккаунт пока не создан. "
                        + "Прогресс сохранён здесь, в браузере. "
                        + "Попробуй ещё раз, когда появится интернет.";
        return;
      }
      ...
      //
      // ЖДЁМ loadAccount. Она асинхронная, и без await всё шло наоборот:
      // подтверждение рисовалось сразу, а через долю секунды приезжал
      // ответ сервера, loadAccount перерисовывала карточку своей подводкой
      // и стирала его. Человек успевал увидеть вспышку текста и не понимал,
      // получилось у него или нет.
      await loadAccount();
