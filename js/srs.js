...
function srsQueue(dictionary, limit = 10) {
  ...
  /* Порядок внутри одинаково просроченных — случайный.
   *
   * Раньше очередь была детерминированной до последнего слова, и это
   * давало неприятный эффект: при словаре из двадцати четырёх слов
   * упражнение на восемь показывало ОДНУ И ТУ ЖЕ восьмёрку каждый раз,
   * пока на неё не ответишь. Открыл, посмотрел, вышел, открыл снова —
   * ровно те же слова. Отсюда и жалоба «слова не меняются».
   *
   * Приоритет расписания при этом цел: сначала всё равно идут самые
   * просроченные и самые трудные, случайность работает только между
   * равными. Слово, которое пора повторить сегодня, не уступит место
   * тому, что ждёт через неделю. */
  const jitter = () => Math.random() - 0.5;

  /* Свежедобавленные: новее — раньше, а при одинаковой дате вперемешку.
   *
   * Тай-брейка тут не было, и это ломало ровно тот случай, ради которого
   * стопка заведена. Репетитор заводит папку и кладёт в неё двенадцать
   * слов одним заходом — у всех двенадцати addedAt совпадает до секунды.
   * Сравнение дат возвращало ноль, сортировка оставляла исходный порядок,
   * и упражнение на восемь брало ПЕРВЫЕ ВОСЕМЬ. Каждый раз. Последние
   * четыре слова папки ученик не видел вообще, сколько бы ни начинал
   * заново. Отсюда «прохожу заново, а слова те же». */
  recent.sort((a, b) => {
    const byDate = String(b.addedAt).localeCompare(String(a.addedAt));
    return byDate !== 0 ? byDate : jitter();
  });
  ...
  overdue.sort((a, b) => {
    ...
    if (weakA !== weakB) return weakB - weakA;            // затем самые проблемные
    return jitter();                                      // среди равных — вперемешку
  });
  fresh.sort(jitter);                                     // новые тоже не по алфавиту
  future.sort((a, b) => {
    const byDue = a.due.localeCompare(b.due);
    return byDue !== 0 ? byDue : jitter();
  });
  ...
  /* Новые слова ВПЛЕТАЮТСЯ в очередь, а не ждут в хвосте.
   *
   * Раньше порядок был такой: сначала все просроченные, потом новые.
   * При словаре в двадцать четыре слова, где восемь ждут повтора,
   * упражнение на восемь вопросов показывало ровно эти восемь — всегда,
   * пока на них не ответишь. Ни одного нового слова в подход не попадало
   * вовсе, и ученик видел одно и то же.
   *
   * Теперь новое слово встаёт примерно каждым четвёртым: подход остаётся
   * прежде всего повторением (расписание важнее), но и новое в нём есть.
   * Так устроены все нормальные тренажёры интервального повторения.
   *
   * Только что добавленные (recent) по-прежнему идут ПЕРЕД просроченными:
   * человек выписал слово, чтобы учить его сейчас. Повторы от этого не
   * теряются — они сразу следом. */
  const due = [...recent,
  ...
  for (let i = 0; i < due.length; i++) {
    out.push(due[i]);
    // каждое четвёртое место отдаём новому слову, пока они есть
    if (fi < takeFresh.length && (i + 1) % 3 === 0) out.push(takeFresh[fi++]);
  }
  while (fi < takeFresh.length) out.push(takeFresh[fi++]);
