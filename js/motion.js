...
function catAnswer(ok) {
  if (ok) {
    ...
    catWrongs++;
    catReact("oops");
    // Слов углового кота на обычный ответ больше нет: рядом с самим ответом
    // говорит инлайн-кот (catAnswerNote в exercises.js) — коротко и по месту.
    // Два говорящих кота на одну ошибку — это хор. Угловой берёт слово
    // только на вехах серии (5/10/20 верных подряд) — редко и празднично.
    // Позы продолжаем менять: живая мимика без слов не спорит с репликой.
  }
  catNap();
}
