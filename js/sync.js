...
function snapshot() {
  return {
    ...
    dictionary: state.dictionary.map(d => ({
      ...
      // Сколько раз на слово был дан ПРОВЕРЕННЫЙ ответ (ввод, выбор,
      // диктант) — его ставит srs.js. Панель репетитора считает домашку
      // сданной именно по нему (db.py, homework_progress).
      //
      // Поля тут не было, и это ломало домашку целиком: ученик честно
      // проходил слова, у себя видел «10 из 10», а у репетитора висело
      // 0 из 10 — и так до тех пор, пока слово не дойдёт до status
      // "learned", а туда ведут четыре успешных повтора по расписанию,
      // то есть дни. Проверено на боевой базе: 116 слов у шести
      // учеников, поле checked — НИ У ОДНОГО.
      checked: d.checked || 0,
...
async function pushProgress() {
  ...
  try {
    ...
    // Серверная озвучка слов (голос Алисы): флаг с каждой синхронизацией —
    // включение и выключение доезжают без перезагрузки страницы
    if (res.ok && typeof res.tts === "boolean") window.SAVELY_TTS = res.tts;
