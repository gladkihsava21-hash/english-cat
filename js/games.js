    ...
    function next() {
      ...
      r.options.forEach((opt, k) => {
        ...
        if (!still()) {
          ...
          // Разгон — по ВЕРНЫМ ответам, а не по номеру раунда. Репетитор
          // просила: начинать медленно и ускорять понемногу с каждым верным
          // ответом, без резкого разгона. Первый шар идёт ~9,5 с, каждый
          // верный ответ снимает полсекунды, ниже 6 с не опускаемся —
          // ошибки скорость не добавляют, поэтому тот, у кого не идёт,
          // не получает вдобавок ещё и гонку. Случайная добавка остаётся:
          // без неё четыре шара летят строем и мишень видно по одному месту.
          const t = Math.min(1, score / 7);
          b.style.animationDuration = (9.5 - t * 3.5 + Math.random() * 1.0).toFixed(1) + "s";
          ...
          b.style.animationDelay = (k * (1.05 - t * 0.4) - 1.1).toFixed(2) + "s";
          // Улетел мимо верхнего края. Если это была мишень — раунд проигран,
          // если чужой шар — просто убираем, чтобы не копились.
          b.addEventListener("animationend", () => {
            // Улетел, пока вкладка была свёрнута, — раунд не проигран.
            //
            // Браузер приостанавливает css-анимации в фоновой вкладке, а
            // при возврате доигрывает их разом. Ученик отвлёкся на другую
            // вкладку, вернулся — и мишень «улетела» задним числом, хотя
            // он её даже не видел. Слово при этом уходило в «забыл» и
            // портило расписание повторений.
            //
