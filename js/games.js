    ...
    function next() {
      ...
      r.options.forEach((opt, k) => {
        ...
        if (!still()) {
          ...
          // Разгон к концу. Восемь одинаковых раундов по девять секунд —
          // это не игра, а восемь одинаковых пауз: к третьему становится
          // скучно, а сложность так и не растёт. Первый раунд идёт
          // спокойно (~9,5 с), последний вдвое быстрее (~4,8 с), между
          // ними ровный подъём. Случайная добавка остаётся: без неё все
          // четыре шара летят строем и мишень видно по одному месту.
          const t = rounds.length > 1 ? i / (rounds.length - 1) : 0;
          b.style.animationDuration = (9.5 - t * 5 + Math.random() * 1.2).toFixed(1) + "s";
          ...
          b.style.animationDelay = (k * (1.05 - t * 0.5) - 1.1).toFixed(2) + "s";
