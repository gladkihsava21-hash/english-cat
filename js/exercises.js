...
function openExercise(id) {
  ...
  if (ex && ex.group === "phrases" && typeof PHRASES === "undefined") {
    // Сброс ДО выхода из функции, а не после.
    //
    // Ниже стоят exMissed = [] и exRoundReset(), но эта ветка до них не
    // доходит: она делает return, пока едет файл с выражениями. Значит
    // первый заход в «Выражения» начинался с журналом ответов, копилкой
    // ошибок и временем старта от ПРЕДЫДУЩЕГО упражнения. В разборе
    // после подхода ученик видел чужие вопросы, а защита от
...
const EX_RUNNERS = {
  ...
  defmatch() {
    let pool = trainPool(4, ["def"]);
    // Английские определения есть только у слов из банка уровней, поэтому
    // из папки со своими словами сюда доходит лишь часть: у Ирины из 28
    // слов — два, и упражнение молча гоняло их по кругу («Верно 2 из 2»,
    // «Ещё раз» — и снова те же). Теперь короткий пул добирается словами
    // уровня из банка, и об этом сказано прямо в подсказке: своих столько,
    // добор такой-то. Это её же логика про диктант: пусть будет
    // дополнительная тренировка, лишь бы честно подписанная.
    ...
    runPairs(pool.map(p => ({ l: p.w, r: p.def, statWord: p.w })), { hint: note });
  },
