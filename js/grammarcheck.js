...
function grammarCheck(text) {
  ...
  if (typeof EN_VERBS !== "undefined") {
    raw.split(/(?<=[.!?])\s+|\n+/).forEach(chunk => {
      ...
      // Двойное замечание на одном месте читается как накрутка: если
      // другая правка уже сработала внутри ЭТОГО предложения («I wan a
      // new bike» уже получило wan → want), про сказуемое молчим —
      // ученик сначала чинит названную ошибку. Не считаются:
      //   • правило про точку (bad с «…») — оно про конец строки,
      //     а не про место ошибки;
      //   • заглавная буква и «i» — однобуквенный bad задевал бы любое
      //     предложение с этой буквой;
      //   • прежние замечания самого правила 15 (good === "") — два
      //     предложения без сказуемого подряд должны быть названы оба.
      // В add() это не вынести: там дедуп по точной паре «было →
      // стало», а пересечение по строкам без спанов задевало бы
      // разные места текста.
      const covered = notes.some(n => n.good !== "" && !n.bad.startsWith("…")
        && (n.bad.toLowerCase().match(/[a-z']+/g) || [])
             .some(t => t.length > 1 && toks.includes(t)));
      if (covered) return;
