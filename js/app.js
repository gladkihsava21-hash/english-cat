...
function initChat() {
  ...
  if (aiKnownOff()) {
    ...
    catSay(`Мяу, ${state.user.name}! Сейчас я отвечаю по правилам, без нейросети. `
  ...
  try {
    ...
    if (!aiWarned) {
      ...
      const why = msg.includes("ai_off")
        ? `Мяу! Сейчас я отвечаю по правилам, без нейросети — со словарём и тренировками это не мешает. ${RULES_HELP}`
