...
def conn():
    ...
    if c is None:
        ...
        # Затирать содержимое удалённых строк, а не просто помечать страницу
        # свободной. Без этого DELETE оставляет байты лежать в файле: после
        # «DELETE FROM call_msgs» все ICE-кандидаты с IP-адресами участников
        # по-прежнему читаются из базы обычным strings.
        #
        # Задаём явно, хотя на нынешнем хостинге и так =1. Значение по
        # умолчанию — свойство СБОРКИ SQLite, а не наше: на маке рядом уже
        # =2 («быстрый» режим, который чистит не всё). Обновят сборку на
        # хостинге — и гарантия уничтожения тихо исчезнет, а мы про это
        # написали в уведомлении в РКН.
        #
        # Цена невелика: база — сотни килобайт, а затирание касается только
        # реально удаляемых страниц.
...
def sync_student(token, state):
    ...
    for d in dictionary[-DICT_MAX_WORDS:]:
        ...
        clean_dict.append({
            ...
            # Счётчик проверенных ответов. По нему панель репетитора
            # решает, сдана ли словарная домашка, — см. homework_progress.
            # Пропускали его здесь ровно так же, как и в snapshot() на
            # клиенте, поэтому до базы он не доезжал ни одним путём.
            "checked": _as_int(d.get("checked"), 0, 0, 9999),
...
#
# Цены подняты 11.09.2026 (решение владельца): 799/1299/2399/4990 и 119 за
# место → 990/1690/2990/5990 и 169. Лестница проверена: каждый тариф
# дешевле предыдущего с доплатами (Старт+5 = 1835 > 1690, Практика+10 =
# 3380 > 2990, Школа+30 = 8060 > 5990) — иначе ступень мертва.
#
# (История) «Практика» стоила 1299, а не 1399, потому что Старт + 5 доплат = 1394:
