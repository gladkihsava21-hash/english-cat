...
ВАЖНО про кэш. Имена файлов шрифтов не версионируются, а отдаются они
с кэшем на год. Для шрифтов это нормально — они не меняются годами, —
но если однажды понадобится ЗАМЕНИТЬ начертание, вернувшимся ученикам
ещё год будет отдаваться старое. Тогда переименуйте файлы (добавьте
что-нибудь в WEIGHT_RANGE-имя) или добавьте ?v= в url() внутри
css/fonts.css. Просто перезалить файл поверх — не сработает.
