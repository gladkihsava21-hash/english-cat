...
# 14, а не 30: каждый деплой кладёт полную копию данных учеников, и за
# месяц их набиралось под сотню. Двух недель хватает, чтобы откатить
# неудачное обновление; дольше — это хранение персональных данных впрок.
KEEP_DAYS=14      # для разовых копий: сама ротация «семь дней» их не касается
