...
# Слова, которые ловятся ТОЛЬКО целиком. Их основы — приставки безобидных
# слов, и как основы они дают ложные срабатывания чаще, чем настоящие:
#   «рак»  → завтракает, ракушка, барак
#   «ад»   → гладить, адрес, ладонь, площадь
#   «раб»  → работа, разработка (убран из основ совсем: рядом есть «рабств»)
SENTENCE_BLOCK_RU_WHOLE = """
рак рака раком ад аду адом
""".split()
...
церков мечет синагог молит молись библи коран тора грех душа рай дьявол сатан
...
опухол болезн зараз инфекц вирус чума эпидеми пандеми диабет инсульт паралич
...
развод изменил предал ненавис ненавид расизм расист рабств голод нищет бездомн убий убит зарезал задушил
...
class SentenceGuard:
    ...
    def __init__(self, word, forms):
        ...
        # Основа должна НАЧИНАТЬ слово, а не встречаться где угодно внутри.
        # Пока сравнивали подстрокой, «раб» ловил «работала», «рак» —
        # «завтракает», «ад» — «гладить»: больше двухсот безобидных
        # предложений отбрасывались как недетские. Правый край оставляем
        # открытым — это именно основа, «убий» обязан ловить «убийцу».
        whole = [w for w in SENTENCE_BLOCK_RU_WHOLE if w not in own_ru_safe]
        self.ru_whole_re = (
            re.compile(r"\b(" + "|".join(whole) + r")\b") if whole else None
        )
        self.ru_re = (
            re.compile(r"\b(" + "|".join(
                sorted((s.replace("ё", "е") for s in self.ru_stems), key=len, reverse=True)
            ) + r")")
            if self.ru_stems else None
        )
    ...
    def check(self, en, ru):
        ...
        hit_ru = self.ru_re.search(low_ru) if self.ru_re else None
        if hit_ru:
            return "не для детей (в переводе): %s" % hit_ru.group(1)
        hit_whole = self.ru_whole_re.search(low_ru) if self.ru_whole_re else None
        if hit_whole:
            return "не для детей (в переводе): %s" % hit_whole.group(1)
        return None
