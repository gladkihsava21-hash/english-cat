...
def build_prompt(payload):
    ...
    # данных об ученике», и это не только обещание, но и разница между
    # «текст без привязки к человеку» и персональными данными ребёнка,
    # уезжающими за границу. По имени кот обращается на сайте сам.
    lines = [
        PERSONA,
        "",
...
SECURITY_HEADERS = [
    ...
    # закрывает перехват на первом заходе (SSL-strip). Срок — год
    # (общепринятое значение): HSTS «запер бы» людей на весь срок только
    # если насовсем перестать отдавать валидный https, чего для сайта не
    # будет; разовая осечка серта блокирует ровно до его починки, и тут
    # что месяц, что год — одно и то же. preload (вшивание в браузеры) НЕ
    # ставим — это отдельный необратимый шаг.
...
class Api:
    ...
    def student_sync(h, p):
        ...
        return {
            ...
            # оставалась пустой. Профиль по ней писал «репетитора нет»
            # человеку, у которого репетитор есть.
            "tutorName": (db.get_tutor_by_id(row["tutor_id"]) or {})["name"]
                         if row["tutor_id"] else "",
    ...
    def student_photo_upload(h, p):
        ...
        # Сколько снимков ученик может держать неразобранными.
        #
        # Предела не было вовсе: на файл 8 МБ, лимит частоты 120 запросов
        # за пять минут — то есть один ученик мог залить под гигабайт за
        # эти пять минут и забить диск общего хостинга всем остальным.
        # Сейчас нейросеть на паузе, значит use_check даже не зовётся и
        # никакая квота не мешает.
        #
        # Считаем только НЕразобранные: репетитор их смотрит и архивирует,
        ...
        if db.photo_count_for_student(row["id"]) >= PHOTO_KEEP_PER_STUDENT:
            return {"ok": False, "error":
                    ...
                    "репетитор посмотрит присланное, и пришли ещё."}
        ...
        # Нейросеть на паузе (AI_PAUSED или нет ключей): ничего не списываем
        # и пакеты не поминаем — use_check иначе советовал бы «попроси
        # репетитора включить проверку», которой сейчас не существует.
        # Фото просто уходит репетитору, статус no_ai клиент показывает
        # как «умная проверка в разработке».
        if db.CHECKS_SUSPENDED:
            db.set_photo_check(photo["id"], "no_ai", None)
            notify_tutor_later(row["tutor_id"], row["name"])
            return {"ok": True, "quota": db.check_state(row["id"]),
    ...
    # Дневные потолки чата — сверх месячной квоты и лимита частоты.
    # Месячная квота (150) не спасает от «сжечь всё за день»: при живом
    # ключе Claude один увлёкшийся ученик выел бы месяц за вечер, а гость
    ...
    @staticmethod
    def chat(h, p):
        # ИИ на паузе или недоступен — честный ai_off сразу, не трогая
        # провайдеров (на машине разработчика есть локальный CLI, и без
        # этой проверки пауза обходилась бы) и не расходуя счётчики.
        if not ai_available():
            return {"ok": False, "error": "ai_off"}
        ...
        if not db.rate_hit(day_key, day_limit, 86400):
            return {"ok": True, "limitReached": True,
                    "reply": "Мур… на сегодня я наговорился — вернусь завтра со свежей головой! 🐈‍⬛ "
                             "А карточки и тренировки со мной всегда, пойдём туда?"}
        if row:
        ...
        try:
            ...
            if row:
                ...
                # гость: вернуть и общий дневной счётчик, иначе чужой сбой
                # провайдера тратит место на витрине всем
                db.rate_refund("chat-guest-global")
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def end_headers(self):
        # При разработке браузер не должен кэшировать css/js — иначе правки
        # не видны до ручной очистки кэша.
        #
        # Расширение берём из ПУТИ, а не из self.path целиком. Раньше стояло
        # self.path.endswith((".css", ...)), и заголовок не ставился НИ РАЗУ:
        # страницы подключают всё через ?v=НОМЕР, то есть путь запроса
        # выглядит как «/css/style.css?v=246» и ни на что не заканчивается.
        # Защита была написана, выглядела рабочей и не работала. Ловится
        # это только тем, что правишь css и не понимаешь, почему его нет
        # на экране, — на это уходит время каждой сессии заново.
        path = urlparse(self.path).path
    ...
    def _servable(path):
        ...
        rules = (
            ...
            # Ролик «как здесь заниматься» на экране регистрации и его
